...
int main(int argc, char *argv[])
{
    ...
    f->attach(std::make_shared<Deflection>(), DataType::Deflection);            //将各个传感器方法注册到该单例模式的文件操作模块内，如果有新增传感器种类
                                                                //均可以使用attach方法注册到文件操作模块中，节省工作量
    f->attach(std::make_shared<strain>(), DataType::Strain);
    f->attach(std::make_shared<Displacement>(), DataType::Displacement);
